...
| [13_flutter_implementation_stack.md](./13_flutter_implementation_stack.md) | Flutter/Dart package stack specification: hooks_riverpod, flutter_hooks, flutter_localizations, fpdart, fl_chart, shimmer, infinite_scroll_pagination, flutter_local_notifications, json_annotation, nanoid, frozen/frozen_annotation, patrol — with provider types, hook patterns, chart configs, DTO serialization, and folder structure |
...
3. `02_domain_model.md` — define all data model classes (all entities are `@frozen` immutable value objects)
4. `13_flutter_implementation_stack.md` — set up the project package stack, folder structure, and code generation tooling
5. `09_data_persistence_and_multi_user.md` — set up the local database layer (use `nanoid` for IDs, `json_annotation` for backup DTOs)
6. `12_algorithms_and_computations.md` — implement pure business logic functions
7. `03_services.md` — implement service layer with `fpdart` error handling and `hooks_riverpod` providers
8. `10_business_rules_and_validation.md` — apply validation rules throughout
9. `06_navigation_and_routing.md` — define routing and navigation
10. `04_pages_catalog.md` + `05_active_session.md` — implement UI pages and components using `flutter_hooks`
11. `11_design_system.md` — apply visual design tokens; use `fl_chart` for analytics, `shimmer` for skeletons, `infinite_scroll_pagination` for lists
12. `07_user_journeys.md` + `08_interaction_state_matrix.md` — verify all interactions are implemented correctly
13. Write `patrol` integration tests covering all 16 major user journeys
