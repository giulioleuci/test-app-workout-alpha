...
> **Flutter implementation note**: All entities and value objects in this document are implemented as **`@frozen` immutable data classes** (using the `frozen`/`frozen_annotation` package). This enforces immutability, generates `copyWith`, `==`, `hashCode`, and `toString` automatically. The `SetModifier` discriminated union is implemented as a `@frozen` sealed class hierarchy. All entity `id` fields are generated using `nanoid` (21-character URL-safe strings). See `13_flutter_implementation_stack.md` for full code patterns.
