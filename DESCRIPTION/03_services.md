...
> **Flutter implementation note**: Services are exposed to the UI layer via **`hooks_riverpod`** providers. Read-only operations are wrapped in `FutureProvider` or `AsyncNotifierProvider`; write operations are exposed as methods on `AsyncNotifier` or `Notifier` classes. All service methods that can fail return `Either<Failure, T>` or `TaskEither<Failure, T>` from the `fpdart` package, enabling type-safe error propagation without exceptions in the domain layer. See `13_flutter_implementation_stack.md` for provider naming conventions and the `Failure` class hierarchy.
