import type { UnresolvedSet } from '@/application/sessionExecution';
  ...
  setUnresolvedSetsState: (state: { open: boolean; sets: UnresolvedSet[] }) => void,
    ...
    const handleEndSession = async (allDone: boolean, title: string, incompleteConfirmDesc: string) => {
        ...
        const doFinish = async () => {
            await sessionExecutionCommands.finishSession(activeSessionId);
            await invalidateSessionContext();
            resetSessionStore();
            navigate('/');
        };

        const validation = await sessionExecutionCommands.validateSessionCompletion(activeSessionId);
    ...
    const handleSkipAllAndFinish = async () => {
        ...
        await sessionExecutionCommands.skipUnresolvedSets(unresolvedSetsState.sets);
        await sessionExecutionCommands.finishSession(activeSessionId);
        await invalidateSessionContext();
        resetSessionStore();
        navigate('/');
    ...
    const handleDiscardSession = (title: string, desc: string) => {
        setAlertConfig({
            ...
            onConfirm: async () => {
                if (!activeSessionId) return;
                await sessionExecutionCommands.discardSession(activeSessionId);
                await invalidateSessionContext();
                resetSessionStore();
                navigate('/');
            },
