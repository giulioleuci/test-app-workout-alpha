...
export function useSessionNavigationHandlers(
    ...
    const handleSwapItems = async (groupId: string, indexA: string, indexB: string) => {
        ...
        await sessionExecutionCommands.swapItems(itemA.item.id, itemB.item.id, itemA.item.orderIndex, itemB.item.orderIndex);
        loadData();
    };
    ...
    const handleSwapGroups = async (indexA: string, indexB: string) => {
        ...
        await sessionExecutionCommands.swapGroups(groupA.group.id, groupB.group.id, groupA.group.orderIndex, groupB.group.orderIndex);
        loadData();
    };
    ...
    const handleActivateUnit = async (targetUnit: DisplayUnit, setViewedSetParams: (val: null) => void) => {
        ...
        // First upcoming group that is not the target, used to insert the displaced current before it
        const firstUpcomingNotTarget = loadedGroups
            .slice(currentGi + 1)
            .find(g => g.group.id !== targetGroup.group.id);

        await sessionExecutionCommands.activateGroup(
            currentGroup.group.id,
            targetGroup.group.id,
            currentGroup.group.orderIndex,
            firstUpcomingNotTarget?.group.orderIndex ?? null,
        );
    ...
    const handleUndoUnitLastSets = async (unit: DisplayUnit, setViewedSetParams: (val: null) => void) => {
        if (unit.type === 'group') {
            await sessionExecutionCommands.uncompleteLastRound(unit.group);
        } else {
            const lastItem = unit.items[unit.items.length - 1];
            await sessionExecutionCommands.uncompleteLastSet(lastItem.item.id);
        }
