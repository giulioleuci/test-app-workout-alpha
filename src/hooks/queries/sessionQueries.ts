...
import { activeSessionLoadingCommands } from '@/composition/activeSessionLoading';
import { exerciseHistoryCommands } from '@/composition/exerciseHistory';
import { loadSuggestionCommands } from '@/composition/loadSuggestions';
import { performanceCommands } from '@/composition/performance';
import type { LoadSuggestionContext } from '@/services/loadSuggestionEngine';
...
export function useExerciseHistory(exerciseId: string, currentSessionId: string, plannedExerciseItemId?: string, occurrenceIndex?: number, filterSameWorkout?: boolean) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => exerciseHistoryCommands.getGroupedHistory(exerciseId, currentSessionId, plannedExerciseItemId, occurrenceIndex, filterSameWorkout),
...
export function useActiveSessionData(activeSessionId: string | null) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => activeSessionLoadingCommands.loadActiveSessionData(activeSessionId),
...
export function useLoadSuggestions(context: LoadSuggestionContext) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => loadSuggestionCommands.getHydratedLoadSuggestions(context),
...
export function usePerformanceTrend(exerciseId?: string, activeSessionId?: string) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => performanceCommands.analyzeExercisePerformance(exerciseId!, activeSessionId!),
