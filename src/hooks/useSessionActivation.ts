...
import type { PendingSessionInfo, SubstitutionPrompt } from '@/application/sessionActivation';
...
export function useSessionActivation() {
  ...
  const doActivate = useCallback(async (
    ...
    try {
      ...
      const wsId = await sessionActivationCommands.activateSession(plannedSessionId, choices);
  ...
  const handleStartSession = useCallback(async (plannedSessionId: string, plannedWorkoutId: string) => {
    ...
    const pending = await sessionActivationCommands.findPendingSessionInfo();
