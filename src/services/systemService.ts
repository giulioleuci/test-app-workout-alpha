...
export const systemService = {
  ...
  setLastActiveUserId: async (userId: string | null): Promise<void> => {
    await databaseLifecycle.updateGlobalAppState({ lastActiveUserId: userId });
  },

  getGlobalAppState: async (): Promise<GlobalAppState | undefined> => {
    return databaseLifecycle.getGlobalAppState();
  },

  updateGlobalAppState: async (update: Partial<GlobalAppState>): Promise<void> => {
    await databaseLifecycle.updateGlobalAppState(update);
  },
