...
export class ExercisePerformanceService {
    ...
    static async analyzeSession(sessionId: string): Promise<void> {
        ...
        if (!hydrated || !hydrated.session.plannedWorkoutId || !hydrated.session.plannedSessionId || !hydrated.session.completedAt) {
            if (hydrated) {
    ...
    private static compareItems(
        ...
        hasRangeConstraint: boolean = false
    ...
    static async analyzeItemOnChange(itemId: string): Promise<void> {
        ...
        if (!session || !session.completedAt || !session.plannedWorkoutId || !session.plannedSessionId) {
