...
export class SessionExecutionService {
  ...
  static async addWarmupSets(itemId: string, warmupSets: WarmupSet[]): Promise<void> {
    const existingSets = await SessionRepository.getSetsByItem(itemId);
    const firstSet = existingSets[0];

    const newSets: SessionSet[] = [];
    let currentNext = firstSet?.orderIndex ?? null;
    const ranks: string[] = [];
  ...
  /**
   * Activates a target group by moving it to the current position, and inserts
   * the displaced current group as the first item in the upcoming list.
   */
  static async activateGroup(
    currentGroupId: string,
    targetGroupId: string,
    currentOrderIndex: string,
    firstUpcomingNotTargetOrderIndex: string | null,
  ): Promise<void> {
    const displacedRank = getRankBetween(currentOrderIndex, firstUpcomingNotTargetOrderIndex);
    await SessionRepository.transaction(async () => {
      await SessionRepository.updateGroup(targetGroupId, { orderIndex: currentOrderIndex });
      await SessionRepository.updateGroup(currentGroupId, { orderIndex: displacedRank });
    });
  }
}
