...
import { generateCsvBlob } from './exerciseCsvDataGateway';
...
export async function buildWorkoutExportData(workoutId: string): Promise<CsvWorkoutExport> {
  ...
  const exerciseIds = [...new Set(items.map(i => i.exerciseId))].filter(Boolean);
...
export function workoutToRows(data: CsvWorkoutExport): (string | number | undefined)[][] {
  ...
  const sortedSessions = [...sessions].sort((a, b) => String(a.orderIndex).localeCompare(String(b.orderIndex)));
  ...
  for (const session of sortedSessions) {
    ...
    const sessionGroups = groups
      ...
      .sort((a, b) => String(a.orderIndex).localeCompare(String(b.orderIndex)));
...
export async function importWorkoutsCsv(
  ...
  for (const planName of planNames) {
    ...
    let lastSessionRank: string | null = null;
    for (const sessionName of sessionNames) {
      const sRows = rowsBySession.get(sessionName.toLowerCase())!;
      const sessionId = nanoid();
      sessionOrderIndex++;
      const sessionRank = getRankBetween(lastSessionRank, null);
      lastSessionRank = sessionRank;
