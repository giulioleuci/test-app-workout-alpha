...
import SetInputWidget from '@/components/session/SetInputWidget';
...
export default function InterleavedGroupRenderer(props: Pick<ExerciseGroupRendererProps, 'lg' | 'gi'>) {
    ...
    const {
        ...
        onCompleteSet, onCompleteScreen, onSkipSet, onSkipRemainingSets: _onSkipRemainingSets,
        onSkipRound, onSkipRemainingRounds, onUncompleteSet, onAddRound,
        setViewedSetParams
    } = useSessionGroupContext();

    const { transientSetValues, setTransientSetValue, clearTransientSetValues } = useActiveSessionStore();
    ...
    const getCurrentValues = (set: SessionSet, item: LoadedItem, roundIdx: number) => {
        return transientSetValues[set.id] || getInitialSetValues(set, item, roundIdx);
    };
    ...
    const handleCompleteMultiScreen = (screen: Screen, roundIdx: number, isLastScreen: boolean) => {
        ...
        for (const itemIdx of screen.itemIndices) {
            ...
            if (set) {
                ...
                clearTransientSetValues(set.id);
            }
        }
                                            ...
                                            <>
                                                ...
                                                <div className="ml-auto flex gap-0.5">
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            const target = screens[activeScreenIndex - 1];
                                                            if (!target) return;
                                                            if (current?.gi === gi && current?.round === roundIdx && target.itemIndices.includes(current.ii)) {
                                                                setViewedSetParams(null);
                                                            } else {
                                                                setViewedSetParams({ gi, ii: target.itemIndices[0], si: roundIdx });
                                                            }
                                                        }}
                                                    >
                                                ...
                                                <div key={screenIdx} className="rounded-lg border border-primary/20 bg-primary/5 p-3">
                                                    ...
                                                    <div className="mt-2 space-y-3">
                                                        ...
                                                        <SetInputWidget
                                                            key={displaySet.id}
                                                            value={currentValues}
                                                            onChange={(val) => setTransientSetValue(displaySet.id, val)}
                                                            ...
                                                            <div key={liItem.item.id} className="rounded-lg border border-muted bg-background p-3 transition-colors">
                                                                ...
                                                                <div className="mt-2 space-y-3">
                                                                    ...
                                                                    <SetInputWidget
                                                                        key={set.id}
                                                                        value={currentValues}
                                                                        onChange={(val) => setTransientSetValue(set.id, val)}
                                                                        sessionSet={set}
                                                                        plannedSet={activePlannedSet}
                                                                        plannedExerciseItem={liItem.plannedItem}
                                                                        exerciseId={liItem.item.exerciseId}
