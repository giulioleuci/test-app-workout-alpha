...
export default function SequentialGroupRenderer(props: Pick<ExerciseGroupRendererProps, 'lg' | 'gi' | 'liItems' | 'itemIndices'>) {
    ...
    const {
        current, viewedSetParams, simpleMode, setCountAdvice: _setCountAdvice,
    ...
    const items = useMemo(() => liItems || [], [liItems]);
    const indices = useMemo(() => itemIndices || [], [itemIndices]);
    ...
    const getCurrentValues = (set: SessionSet, li: LoadedItem, si: number) => {
        return transientSetValues[set.id] || getInitialSetValues(set, li, si);
    };
    ...
    const handleComplete = (globalSi: number) => {
        ...
        if (displaySet && currentValues) {
            ...
            clearTransientSetValues(displaySet.id);
        }
    };
