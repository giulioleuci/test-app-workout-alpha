    ...
    <div className={cn("flex items-center gap-2", className)}>
      ...
      <div className={cn(!iconOnly && "w-32")}>
        <Select value={language} onValueChange={(v) => changeLanguage(v as 'en' | 'it' | 'es' | 'fr' | 'zh')}>
          {iconOnly ? (
            <SelectTrigger className="h-9 w-9 justify-center border-none bg-transparent p-0 shadow-none hover:bg-accent hover:text-accent-foreground focus:ring-0 data-[state=open]:bg-accent data-[state=open]:text-muted-foreground [&>svg:last-child]:hidden">
