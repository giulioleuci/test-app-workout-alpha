...
import type { WorkoutVolumeAnalysis, VolumeEntry } from '@/services/volumeAnalyzer';
...
export default function VolumeAnalysisDialog({ workoutId, open, onOpenChange }: VolumeAnalysisDialogProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void Promise.all([
      volumeCommands.analyzeWorkoutVolume(
        workoutId,
        (k) => {
      ...
      durationCommands.estimateWorkoutDuration(workoutId),
    ]).then(async ([r, wd]) => {
      setAnalysis(r);
      setWorkoutDuration(wd);
      const durEntries = await Promise.all(
        r.sessions.map(async s => [s.sessionId, await durationCommands.estimateSessionDuration(s.sessionId)] as const)
      );
        ...
        {analysis && !loading && (
          <Tabs defaultValue="total">
            <TabsList className="flex h-auto w-full flex-wrap gap-1">
              <TabsTrigger value="total" className="text-body-sm h-auto min-w-[120px] flex-1 whitespace-normal break-words py-2">{t('analytics.fullPlan')}</TabsTrigger>
              {analysis.sessions.map(s => (
                <TabsTrigger key={s.sessionId} value={s.sessionId} className="text-body-sm h-auto min-w-[120px] flex-1 whitespace-normal break-words py-2">
                  {s.sessionName}
                </TabsTrigger>
              ))}
            </TabsList>
