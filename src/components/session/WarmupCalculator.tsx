...
import { Flame, Calculator, Minus, Plus, PlusCircle } from 'lucide-react';
import { useTranslation } from 'react-i18next';

import { calculateUserWarmup, type WarmupSet } from '@/application/warmups';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { warmupCommands } from '@/composition/warmups';
...
interface WarmupCalculatorProps {
  ...
  onInsertSets?: (sets: WarmupSet[]) => void;
}

export default function WarmupCalculator({ workingWeight, exercise, workoutSessionId, userWarmupConfig, trigger, onInsertSets }: WarmupCalculatorProps) {
  ...
  useEffect(() => {
    const calculate = async () => {
      ...
      if (!isNaN(weight) && weight > 0) {
        const generated = await warmupCommands.generateWarmup(weight, exercise, workoutSessionId || undefined);
        setSets(generated);
  ...
  const handleInsert = () => {
    const selectedSets = activeTab === "manual" && hasUserConfig ? userSets : sets;
    if (onInsertSets && selectedSets.length > 0) {
      onInsertSets(selectedSets);
      setOpen(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="overflow-y-auto sm:w-full sm:max-w-md" style={{ maxHeight: '90vh', width: '95vw' }}>
        <DialogHeader>
