...
export default function LoadSuggestionDialog({ 
  ...
  const { data: historicalData, isLoading } = useQuery({
    queryKey: ['historicalRecommendations', exerciseId, plannedSet?.id],
    queryFn: async () => {
    ...
    <div className={cn(
      ...
      <div className="flex flex-col gap-1 overflow-hidden">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-bold whitespace-nowrap">{recommendation.load} {t('units.kg')}</span>
        </div>
      ...
      <Button size="sm" variant={isPreferred ? "default" : "secondary"} className="h-8 gap-1.5 shrink-0" onClick={() => onApply(recommendation.load)}>
