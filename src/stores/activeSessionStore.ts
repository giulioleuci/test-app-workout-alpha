...
interface ActiveSessionState {
  ...
  // Unsaved values for all sets (persisted across sessions)
  transientSetValues: Record<string, SetInputValue>;
  ...
  setTransientSetValue: (setId: string, values: SetInputValue) => void;
  clearTransientSetValues: (setId?: string) => void;
  reset: () => void;
}
...
export const useActiveSessionStore = create<ActiveSessionState>()(
  persist(
    (set, get) => ({
      ...
      setTransientSetValue: (setId, values) =>
        set((state) => ({
          transientSetValues: { ...state.transientSetValues, [setId]: values }
        })),

      clearTransientSetValues: (setId) =>
        set((state) => {
          if (setId) {
            const { [setId]: _, ...rest } = state.transientSetValues;
            return { transientSetValues: rest };
          }
          return { transientSetValues: {} };
        }),

      reset: () => set(initialState),
    }),
    {
    name: `active-session-store-${systemCommands.getCurrentUserId()}`,
      partialize: (state) => ({
        ...
        transientSetValues: state.transientSetValues,
      }),
    }
  )
);
