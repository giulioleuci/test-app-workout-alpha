...
describe('ExerciseHistoryButton', () => {
  ...
  it('loads and displays history for an exercise', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('10 rep × 100kg')).toBeInTheDocument();
      expect(screen.getByText(/01 gen 2023/)).toBeInTheDocument();
    });
  });
  ...
  it('correctly sorts items by group order then item order (occurrence logic)', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('5 rep × 50kg')).toBeInTheDocument();
      expect(screen.queryByText('8 rep × 80kg')).not.toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.getByText('8 rep × 80kg')).toBeInTheDocument();
      expect(screen.queryByText('5 rep × 50kg')).not.toBeInTheDocument();
    });
  });
});
