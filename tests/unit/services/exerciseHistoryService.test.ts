...
describe('exerciseHistoryService', () => {
  ...
  describe('getGroupedHistory', () => {
    it('returns empty array if no items found', async () => {
      ...
      const result = await exerciseHistoryCommands.getGroupedHistory('ex1', 'curr1');
    ...
    it('returns grouped history', async () => {
      ...
      const result = await exerciseHistoryCommands.getGroupedHistory(exerciseId, currentSessionId);
