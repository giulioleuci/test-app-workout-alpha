...
import { analyzeItemsFromData, analyzeItemsRaw, ItemWithContext } from '@/services/volumeAnalyzer';
...
describe('volumeAnalyzer', () => {
  ...
  describe('analyzeSessionVolume', () => {
    it('returns empty analysis if session not found', async () => {
      ...
      const result = await volumeCommands.analyzeSessionVolume('s1', 'Session 1', muscleLabelFn, groupLabelFn, patternLabelFn);
  ...
  describe('analyzeWorkoutVolume', () => {
    it('analyzes full workout volume including overlap', async () => {
      ...
      const result = await volumeCommands.analyzeWorkoutVolume('w1', muscleLabelFn, groupLabelFn, patternLabelFn);
  ...
  describe('getSessionVolumeAndDuration', () => {
    it('returns null if session not found', async () => {
      vi.mocked(WorkoutPlanRepository.getHydratedPlannedSession).mockResolvedValue(null);
      vi.mocked(durationCommands.estimateSessionDuration).mockResolvedValue({ minSeconds: 0, maxSeconds: 0 });
      const result = await volumeCommands.getSessionVolumeAndDuration('s1');
