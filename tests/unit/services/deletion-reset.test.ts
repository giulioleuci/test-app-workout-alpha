...
describe('Account Deletion and App Reset', () => {
  ...
  it('userService.deleteCurrentUser should clear active session and delete user data', async () => {
    ...
    const user = await userCommands.createUser('Delete Me');
    ...
    const lastActive = await systemCommands.getLastActiveUserId();
  ...
  it('SystemMaintenanceService.resetWholeApplication should clear EVERYTHING', async () => {
    ...
    await userCommands.createUser('User 1');
    await userCommands.createUser('User 2');
    localStorage.setItem('some-setting', 'value');

    // 2. Perform reset
    await maintenanceCommands.resetWholeApplication();
