...
import { exerciseVariantCommands } from '@/composition/exerciseVariants';
import type { Exercise } from '@/domain/entities';
import { ExerciseType, MovementPattern, CounterType, Equipment, Muscle } from '@/domain/enums';
import { exercisesToCsv, parseExerciseCsv } from '@/infrastructure/csv/exerciseCsvDataGateway';
...
describe('exerciseVariantService', () => {
  ...
  describe('addVariant', () => {
    it('links two exercises bidirectionally', async () => {
      ...
      await exerciseVariantCommands.addVariant(a.id, b.id);
    ...
    it('throws on self-link', async () => {
      ...
      await expect(exerciseVariantCommands.addVariant(a.id, a.id)).rejects.toThrow();
    });
    ...
    it('throws if exercise does not exist', async () => {
      const a = makeExercise();
      await db.exercises.add(a);
      await expect(exerciseVariantCommands.addVariant(a.id, 'nonexistent')).rejects.toThrow();
    });
  });
  ...
  describe('removeVariant', () => {
    it('removes link from both exercises', async () => {
      ...
      await exerciseVariantCommands.removeVariant(a.id, b.id);
    ...
    it('is safe to call when link does not exist', async () => {
      ...
      const b = makeExercise();
      await db.exercises.bulkAdd([a, b]);
      await expect(exerciseVariantCommands.removeVariant(a.id, b.id)).resolves.not.toThrow();
    });
  });
  ...
  describe('getVariants', () => {
    it('returns variant exercises', async () => {
      ...
      const variants = await exerciseVariantCommands.getVariants(a.id);
  ...
  describe('removeExerciseFromAllVariants', () => {
    it('cleans up all references when exercise is deleted', async () => {
      ...
      await exerciseVariantCommands.removeExerciseFromAllVariants(a.id);
