...
import {
  DetectBackupConflicts, ExportBackup, ImportBackup,
  type BackupDataPort, type BackupFileGateway, type BackupSchema,
} from '@/application/backup';
import { backupDataGateway } from '@/infrastructure/backup/backupGateways';
...
describe('backupService', () => {
  ...
  it('exportAll returns all tables with correct schema', async () => {
    const downloads: BackupSchema[] = [];
    const backup = await exportBackup(downloads);
  ...
  it('exportTables exports only selected tables', async () => {
    const backup = await exportBackup([], ['exercises']);
    expect(backup.data.exercises).toHaveLength(2);
  ...
  it('importData with "ignore" skips conflicting records', async () => {
    ...
    const result = await importBackup(backup, 'ignore', conflicts);
  ...
  it('importData with "overwrite" replaces conflicting records', async () => {
    ...
    const conflicts = await detectConflicts(backup);
    const result = await importBackup(backup, 'overwrite', conflicts);
  ...
  it('importData with "copy" creates new IDs for conflicts', async () => {
    ...
    const conflicts = await detectConflicts(backup);
    const result = await importBackup(backup, 'copy', conflicts);
...
const noOpFiles: BackupFileGateway = {
  download: async () => undefined,
  pick: async () => ({ data: '', name: '' }),
};

async function exportBackup(downloads: BackupSchema[], tables?: Parameters<ExportBackup['execute']>[0]) {
  const files: BackupFileGateway = { ...noOpFiles, download: async backup => { downloads.push(backup); } };
  await new ExportBackup(backupDataGateway, files).execute(tables);
  return downloads[0]!;
}

function detectConflicts(backup: BackupSchema) {
  return new DetectBackupConflicts(backupDataGateway).execute(backup);
}

function importBackup(backup: BackupSchema, strategy: 'copy' | 'ignore' | 'overwrite', conflicts: Awaited<ReturnType<typeof detectConflicts>>) {
  return new ImportBackup(backupDataGateway).execute(backup, strategy, conflicts);
}
