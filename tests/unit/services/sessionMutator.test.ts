...
import {
  sessionMutationCommands as sessionMutator,
} from '@/composition/sessionMutation';
import type {
  Exercise,
  WorkoutSession,
...
describe('SessionMutator', () => {
  ...
  describe('swapExercise', () => {
    it('should replace the exerciseId on the item', async () => {
      ...
      await sessionMutator.swapExercise(itemId, EX_SQUAT);
    ...
    it('should preserve originalExerciseId across a double swap', async () => {
      ...
      await sessionMutator.swapExercise(itemId, EX_SQUAT);
      await sessionMutator.swapExercise(itemId, EX_DEADLIFT);
    ...
    it('should record an ExerciseSubstitution when plannedWorkoutId is provided', async () => {
      ...
      await sessionMutator.swapExercise(itemId, EX_SQUAT, plannedWorkoutId);
    ...
    it('should NOT record substitution when item has no plannedExerciseItemId', async () => {
      ...
      await sessionMutator.swapExercise(itemId, EX_SQUAT, 'pw-1');
    ...
    it('should throw if item does not exist', async () => {
      await expect(sessionMutator.swapExercise('nonexistent', EX_SQUAT)).rejects.toThrow();
    });
  });
  ...
  describe('addExercise', () => {
    it('should create group, item, and 3 fallback sets at the target position', async () => {
      const result = await sessionMutator.addExercise(SESSION_ID, EX_BENCH, 0);
    ...
    it('should shift existing groups when inserting at an occupied index', async () => {
      ...
      // Insert at index 1
      const result = await sessionMutator.addExercise(SESSION_ID, EX_CURL, 1);
    ...
    it('should accept a custom groupType', async () => {
      const result = await sessionMutator.addExercise(SESSION_ID, EX_BENCH, 0, ExerciseGroupType.Warmup);
  ...
  describe('addSuperset', () => {
    it('should create a Superset group with multiple items, each with 3 sets', async () => {
      const result = await sessionMutator.addSuperset(SESSION_ID, [EX_BENCH, EX_SQUAT, EX_CURL], 0);
    ...
    it('should shift existing groups', async () => {
      ...
      const result = await sessionMutator.addSuperset(SESSION_ID, [EX_BENCH, EX_SQUAT], 0);
    ...
    it('should allow a custom groupType', async () => {
      const result = await sessionMutator.addSuperset(SESSION_ID, [EX_BENCH, EX_SQUAT], 0, ExerciseGroupType.Circuit);
  ...
  describe('removeExercise', () => {
    it('should delete the item and its sets', async () => {
      ...
      await sessionMutator.removeExercise(itemId);
    ...
    it('should re-compact orderIndex on remaining groups after group deletion', async () => {
      ...
      await sessionMutator.removeExercise('i1');
    ...
    it('should throw if item does not exist', async () => {
      await expect(sessionMutator.removeExercise('nonexistent')).rejects.toThrow();
    });
  });
  ...
  describe('validateSessionCompletion', () => {
    it('should return valid when all sets are completed', async () => {
      ...
      const result = await sessionMutator.validateSessionCompletion(SESSION_ID);
