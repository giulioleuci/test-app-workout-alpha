...
const getHistoryPage = (page: number, pageSize: number) => historyCommands.getHistoryPage(page, pageSize);
const getHistoryDetail = (id: string) => historyCommands.getHistoryDetail(id);
const getFilteredHistory = (filters: Parameters<typeof historyCommands.getFilteredHistory>[0]) => historyCommands.getFilteredHistory(filters);
const deleteHistorySession = (id: string) => historyCommands.deleteHistorySession(id);
const updateHistorySessionMeta = (id: string, updates: Parameters<typeof historyCommands.updateHistorySessionMeta>[1]) => historyCommands.updateHistorySessionMeta(id, updates);
