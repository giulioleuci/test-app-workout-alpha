...
import { calculateUserWarmup, getWarmupExerciseType, WarmupUseCases } from '@/application/warmups';
...
describe('WarmupCalculator Service', () => {
  const warmups = new WarmupUseCases({
    getExercisesInSession: () => Promise.resolve(null),
    getLatestBodyWeight: () => Promise.resolve(undefined),
  });
  ...
  describe('generateWarmup', () => {
    ...
    it('should generate isolation warmup', async () => {
      ...
      const sets = await warmups.generateWarmup(100, mockIsolationExercise);
    ...
    it('should generate compound upper high stress warmup', async () => {
      ...
      const sets = await warmups.generateWarmup(100, mockExercise, undefined, 80);
    ...
    it('should generate compound lower medium stress warmup', async () => {
      ...
      const sets = await warmups.generateWarmup(100, mockLowerExercise, undefined, 120);
