...
describe('ActiveSession Component', () => {
  ...
  it('renders upcoming and completed exercises accordions with summaries', async () => {
    ...
    await waitFor(() => {
      ...
      // Upcoming accordion
      expect(screen.getByText(/Prossimi Esercizi/i)).toBeInTheDocument(); // "Upcoming Exercises"
      // Completed accordion
      expect(screen.getByText(/Esercizi Completati/i)).toBeInTheDocument(); // "Completed Exercises"
    });
  });
  ...
  it('advances through a Superset using round-robin order', async () => {
    ...
    const completeButtons = screen.getAllByRole('button', { name: /Completa/i });
