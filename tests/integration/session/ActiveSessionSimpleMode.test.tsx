...
describe('ActiveSession Simple Mode', () => {
  ...
  it('hides advanced features in Simple Mode', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Sessione attiva/i)).toBeInTheDocument();
    });
  ...
  it('hides RPE-based suggestions and removes RPE from reasoning in Simple Mode', async () => {
    ...
    await waitFor(() => {
      suggestionBtn = screen.getByText(/Suggerimento carico/i);
    ...
    await waitFor(() => {
      // Should show "Ultima Sessione: 100 kg × 5 rep" (translated labels)
      // And NOT "@ RPE 9"
      const suggestions = screen.getAllByText(/Ultima Sessione/i);
  ...
  it('shows advanced features when Simple Mode is OFF', async () => {
    ...
    expect(screen.getAllByText(/Cedimento/i)[0]).toBeInTheDocument();
